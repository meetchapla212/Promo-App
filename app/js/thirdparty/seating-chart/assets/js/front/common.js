        ...
        if (supportsTouch !== true) {
            $('body').on('hover', '.tc_set_seat, .tc-object-selectable.ui-selectee', function(event) {
                if (!$(event.target).is(".tc_seat_unavailable")) {
                    ...
                    $(this).append('<div class="tc-ticket-info-wrap" style="position: absolute; -webkit-animation-name: fadeIn; -webkit-animation-duration: 0.2s;  animation-name: fadeIn; animation-duration: 0.2s;display:block;"><div class="tc-arrow-up"></div><div class="tc-ticket-info-inside"><ul><li><span><strong class="tc_ticket_type_price_hover_wrapper">' + ticket_type_price + '</strong> - </span>' + ticket_type_title + '</li></ul>' + ticket_seat_number + '<div class="ticket_sale_end">Ticket sale has ended.</div></div></div>');
                }
            });

            $('body').on('mouseout', '.tc_set_seat, .tc-object-selectable.ui-selectee', function() {
                $(".tc-wrapper").find('div.tc-ticket-info-wrap').remove();
            });
        }
