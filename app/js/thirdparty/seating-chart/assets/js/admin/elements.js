jQuery(document).ready(function($) {
    ...
    $('body').on('click', '.tc-element-group-wrap .tc-group-controls .tc-icon-edit', function() {
        ...
        $(".tc-sidebar .md-objects").find(".dataContent").addClass("activeContent");
        $(".tc-sidebar .md-objects").find(".expandCollapse").addClass("active");
        $(this).off("click");
    });
    ...
    window.tc_element = {
        ...
        create_mixed: function(iconValue, titleValue, leftValue, topValue, plusValue, areaWidth) {
            var grid_size = 5;
            var icon = iconValue;
            ...
            if (icon_background_color !== '') {
                background_color_style = 'style="width: ' + areaWidth + 'px;background-color:' + icon_background_color + '; -moz-box-shadow: 0 0 8px 1px rgba(0, 0, 0, 0.05); -webkit-box-shadow: 0 0 8px 1px rgba(0, 0, 0, 0.05);box-shadow: 0 0 8px 1px rgba(0, 0, 0, 0.05);"';
            }
            ...
            text_html += '<div class="tc-element-group" ' + background_color_style + ' >'; //tc-group-background
            text_html += '<div class="tc-heading ' + empty_class + '"><h3 ' + color_style + '>' + title + '</h3></div>';
            text_html += '<div class="tc-object" style="width: ' + areaWidth + 'px;"><span class="' + icon + '" ' + color_style + '></span></div><!-- .tc-object -->';
            text_html += '<div class="tc-group-controls"><span class="tc-icon-edit"></span><span class="tc-icon-trash"></span><span class="tc-icon-copy"></span></div>';
            text_html += '</div></div>';
            var text = text_html;
            var new_element = tc_element.add_to_canvas(text, new Array(1, 1), true);

            tc_controls.align_center($(new_element), leftValue, topValue, plusValue);
