jQuery(document).ready(function($) {
    ...
    $('body').on('click', '.tc-group-seats .tc-group-controls .tc-icon-edit', function() {
    ...
    $('body').on('click', '.tc-group-seats .tc-group-controls .tc-icon-assign', function(e) {
        if (!toggleValue) {
            $(this).parent().parent('.tc-seat-group').find('.ui-selectable').selectable({
                filter: '.tc_seat_unit',
                cancel: '.tc_seat_reserved',
                disabled: false
            });
            $(this).parent().parent('.tc-seat-group').find(".tc_seat_unit").addClass('tc_set_seat');
            $(".tc-sidebar").find(".dataContent").removeClass("activeContent");
            $(".tc-sidebar").find(".expandCollapse").removeClass("active");
            $(".tc-sidebar .md-section").find(".dataContent").addClass("activeContent");
            $(".tc-sidebar .md-section").find(".expandCollapse").addClass("active");
            ...
            $(this).parent().parent('.tc-seat-group').find(".tc_seat_unit").removeClass('tc_set_seat');
            toggleValue = false;
        }
        e.preventDefault();
        $(this).off("click");
    ...
    window.tc_seats = {
            ...
            if (rows == rows_prev && cols == cols_prev) {
                    ...
                    if (rows > rows_prev) { //add rows
                        ...
                        for (i = 1; i < (parseInt(rows_to_add) + 1); i++) {
                            ...
                            for (j = 1; j < (parseInt(cols_prev) + 1); j++) {
                                seat_group_html += '<span class="tc_seat_unit ui-selectee" id="tc_seat_' + next_part_no + '_' + no + '"></span>';
                                no++;
                            }
                ...
                if (cols != cols_prev) {
                    if (cols > cols_prev) {
                        ...
                        var group_size = (parseInt($('#tc_square_size').val()) * parseInt(cols) + 40);
        ...
        create_group: function() {
            ...
            var textValue = "'section'";
            seat_group_html += '</div><!-- .tc-group-content --><div class="tc-group-controls"><span class="tc-icon-edit"></span><span class="tc-icon-trash"></span><span class="tc-icon-assign"></span></div></div><!-- .tc-seat-group --><div class="tc-clear"></div></div>';
            ...
            try {
                ...
                $(new_element).draggable({
                    ...
                    handle: $(new_element).find('.tc-seat-group'),
        ...
        create_mixed_group: function(value, count, topValue) {
            var rows = $('#tc_seat_add_mixed_rows').val();
            var cols = $('#tc_seat_add_mixed_cols').val();
            var group_size = (parseInt($('#tc_square_size').val()) * parseInt(cols) + 40);
            var header_title = value;
            var seat_group_html = '';
            var grid_size = 5;
            var next_part_no = tc_controls.next_part_number();
            var no = 1;
            ...
            seat_group_html += '<div class="tc-group-wrap tc-group-seats" style="width:' + group_size + 'px; left: ' + group_size * value + 'px;" data-init-width=' + group_size + '><div class="tc-seat-group tc-group-background">';
            seat_group_html += '<div class="tc-heading ' + empty_class + '"><h3>' + header_title + '</h3></div><!-- .tc-heading --><div class="tc-group-content">';
            ...
            var group = seat_group_html;
            var new_element = tc_seats.add_to_canvas(group, new Array(1, 1), true);
            var leftCount = parseInt(value) - parseInt(count);
            tc_controls.align_center($(new_element), leftCount - 1, -topValue, 106);
            ...
            try {
                ...
                $(new_element).draggable({
                    grid: [grid_size * window.tc_seat_zoom_level * value, grid_size * window.tc_seat_zoom_level],
                    handle: $(new_element).find('.tc-seat-group'),
                    start: function(event, ui) {
                        ui.position.left = 0;
                        ui.position.top = 0;
        ...
        create_section_rows_group: function(column, rows, value, count, topValue, plusValue) {
            var rows = rows;
            var cols = column;
            var group_size = (parseInt($('#tc_square_size').val()) * parseInt(cols) + 40);
            var header_title = value;
            ...
            var new_element = tc_seats.add_to_canvas(group, new Array(1, 1), true);
            var leftCount = parseInt(value) - parseInt(count);
            tc_controls.align_center($(new_element), leftCount - 1, topValue, plusValue);
        ...
        init: function() {
            ...
            $('.tc-group-wrap.tc-group-seats').each(function() {
                ...
                $(this).draggable({
                    grid: [grid_size * window.tc_seat_zoom_level, grid_size * window.tc_seat_zoom_level],
                    handle: $(this).find('.tc-seat-group'),
                    start: function(event, ui) {
                        ui.position.left = 0;
                        ui.position.top = 0;
                    },
