...
Message.prototype.renderMessage = function(message, setAsFirst) {
    ...
    function loadAttachmentListenser(e) {
        var img = e.target,
    ...
    function errorAttachmentListener(e) {
        var img = e.target,
    ...
    if (message.attachments.length) {
        ...
        for (let i = 0; i < images.length; i++) {
            const j = i;
            images[j].addEventListener('load', loadAttachmentListenser);
            images[j].addEventListener('error', errorAttachmentListener);
        }
    }
