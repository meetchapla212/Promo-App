...
function buttonDisabled() {
    var x = document.querySelector(".dialog_name");
    if (x.value === '') {
        document.querySelector('.j-create_dialog_btn').disabled = true;
    } else {
        document.querySelector('.j-create_dialog_btn').disabled = false;
    }
}
    ...
    return new Promise(function(resolve, reject) {
        ...
        QB.chat.dialog.list(filter, function(err, resDialogs) {
            ...
            _.each(dialogs, function(dialog) {
                ...
                if (dialog.type !== 2) {
                    promises.push(userModule.getUserImageById(self._cache[dialog._id].jidOrUserId));
                }
            });

            return Promise.all(promises)
                .then((userToImageMappings) => {
                    let imageMapper = {};
                    if (userToImageMappings) {
                        userToImageMappings = userToImageMappings.filter(u => u != null);
                        userToImageMappings.forEach((m) => {
                            if (m) {
                                imageMapper[m.id] = m.profile_url;
                            }
                        });
                    }

                    _.each(dialogs, function(dialog) {
                        let oldDialogCached = self._cache[dialog._id];
                        if (imageMapper && dialog.type !== 2) {
                            oldDialogCached.profile_url = imageMapper[oldDialogCached.jidOrUserId];
                            self._cache[dialog._id] = oldDialogCached;
                        }
...
Dialog.prototype.decreaseUnreadCounter = function(dialogId) {
    ...
    // decrease from Cookie also
    let userUnreadMessageCount = Cookie.find("userUnreadMessageCount");
    if (userUnreadMessageCount) {
        Cookie.create('userUnreadMessageCount', dialog.unread_messages_count, 120);
    }
...
Dialog.prototype.renderMessages = function(dialogId) {
    ...
    if (!document.forms.send_message) {
        ...
        self.userIcon = document.querySelector('.j-dialog-user-icon');
        ...
        if (dialog.type === CONSTANTS.DIALOG_TYPES.GROUPCHAT) {
            ...
            self.userIcon.classList.remove('m-type_chat');
            removeClassByPrefix(self.userIcon, 'm-user__img_');
            self.userIcon.classList.add('m-user__img_' + dialog.color);
            self.userIcon.classList.add('m-type_group');
        } else {
            self.editLink.classList.add('hidden');
            self.userIcon.classList.add('m-type_chat');
            removeClassByPrefix(self.userIcon, 'm-user__img_');
            self.userIcon.classList.add('m-user__img_' + dialog.color);
            self.userIcon.classList.remove('m-type_group');
        }
    ...
    self.checkCachedUsersInDialog(dialogId).then(function() {
        ...
        if (dialog && dialog.messages.length) {
            let oldMessageId = '';
            for (var i = 0; i < dialog.messages.length; i++) {
                if (oldMessageId != dialog.messages[i]._id) {
                    oldMessageId = dialog.messages[i]._id;
...
Dialog.prototype.createDialog = function(params) {
    ...
    QB.chat.dialog.create(params, function(err, createdDialog) {
        if (err) {
            ...
            if (app.user.first_name == 'null' || app.user.first_name == undefined) {
                message_body = (app.user.name || app.user.login) + ' created new dialog with: ';
            } else {
                message_body = app.user.first_name + ' ' + app.user.last_name + ' created new dialog with: ';
            }

            _.each(occupants, function(occupantId) {
                if (userModule._cache[occupantId].first_name == 'null' || userModule._cache[occupantId].first_name == undefined) {
                    occupantName = userModule._cache[occupantId].name;
                } else {
                    occupantName = userModule._cache[occupantId].first_name + ' ' + userModule._cache[occupantId].last_name;
                }
                occupants_names.push(occupantName);
            });
...
Dialog.prototype.updateDialog = function(updates) {
    ...
    if (updates.title) {
        if (updates.title !== dialog.name) {
            ...
            if (app.user.first_name == 'null' || app.user.first_name == undefined) {
                updatedMsg.body = app.user.first_name + ' ' + app.user.last_name + ' changed the conversation name to "' + updates.title + '".';
            } else {
    ...
    if (!updates.remove && updates.userList) {
        ...
        if (newUsers.length) {
            ...
            if (app.user.first_name == 'null' || app.user.first_name == undefined) {
                ...
                updatedMsg.body = app.user.first_name + ' ' + app.user.last_name + ' adds ' + usernames.join(', ') + ' to the conversation.';
            }
            updatedMsg.extension.occupants_ids_added = newUsers.join(',');
        }
    } else {
        updatedList = _removeOldUsers();
        if (updates.userList && updates.remove) {
            toUpdateParams.pull_all = {
                occupants_ids: updatedList
            };

            var updateusers = updatedList.map(function(userId) {
                return userModule._cache[userId].name || userId;
            });

            self._cache[dialogId].users = self._cache[dialogId].users.filter(function(el) {
                return updatedList.indexOf(el) < 0;
            });
            if (app.user.first_name == 'null' || app.user.first_name == undefined) {
                updatedMsg.body = (app.user.name) + ' removed ' + updateusers.join(', ') + ' to the conversation.';
            } else {
                updatedMsg.body = app.user.first_name + ' ' + app.user.last_name + ' removed ' + updateusers.join(', ') + ' to the conversation.';
            }
        }
    }
    ...
    function _removeOldUsers() {
        return updates.userList.filter(function(occupantId) {
            return dialog.users.indexOf(occupantId) >= 0;
        });
    }
...
Dialog.prototype.quitFromTheDialog = function(dialogId) {
    ...
    function _notuyfyUsers() {
        var msg = {
            type: 'groupchat',
            body: ((app.user.first_name == 'null' || app.user.first_name == undefined) ? app.user.name : app.user.first_name + ' ' + app.user.last_name) + ' left the chat.',
            extension: {
                save_to_history: 1,
                dialog_id: dialog._id,
                notification_type: 2,
