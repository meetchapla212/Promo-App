...
module.exports.handler = async (event, context, callback) => {
    ...
    try {
        ...
        let getCompletePayoutsOfEvents = `SELECT _event_id FROM event_payout_master where _user_id = ${user.id} AND payout_created >= ${start_date_ms} AND payout_created <= ${end_date_ms} AND payout_status = 'complete' ORDER BY payout_created ASC `;
        let completePayoutsOfEvents = await DBManager.runQuery(getCompletePayoutsOfEvents);
        if (completePayoutsOfEvents && completePayoutsOfEvents.length > 0) {
            return Promise.all(completePayoutsOfEvents.map(async completePayout => {
                var eventId = completePayout._event_id;
                ...
                response = { success: true, message: successMessages.USER_COMPLETED_PAYOUT_EVENTS, totalPayouts: userPendingPayoutResult.length, data: userPendingPayoutResult };
            ...
            response = { success: true, message: 'User have not any completed payout event', totalPayouts: 0, data: [] };
            return awsRequestHelper.respondWithSimpleMessage(200, response);
        }
    } catch (error) {
