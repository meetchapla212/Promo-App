...
const notifyUser = async (event, notifyAttendee, organiser, notifyData, orderId, QRCodes) => {
    if (notifyAttendee.email && notifyAttendee.email !== "" && notifyAttendee.is_email_verified && notifyAttendee.is_email_verified == 1) {
        ...
        let tmpl = fs.readFileSync("./lambda/emailtemplates/notify_attendee_when_ticket_purchase.html", "utf8");
        let mailSubject = `Ticket Purchase for ${event.event_name}`;
        let event_name = event.event_name.replace(/\s+/g, "-").toLowerCase();
        let event_url = `${process.env.UI_BASE_URL}/eventdetails/${event_name}/${event.event_id}`;
        let templateVars = Object.assign({
...
const notifyUserByPushNotification = async (userId, eventName, totalQty) => {
    var userDeviceTokenData = await getUserDeviceToken(userId);
    if (userDeviceTokenData.length > 0) {
        var { token } = userDeviceTokenData[0];

        let PURCHASED_TICKETS_TO_EVENT = emailAndPushNotiTitles.PURCHASED_TICKETS_TO_EVENT;
        PURCHASED_TICKETS_TO_EVENT = PURCHASED_TICKETS_TO_EVENT
            .replace('<totalQty>', totalQty)
            .replace('<eventName>', eventName);

        var message = {
            title: emailAndPushNotiTitles.TICKET_PURCHASED,
            body: PURCHASED_TICKETS_TO_EVENT,
...
//notify user on transaction failed when you tried to buy tickets for an event
const notifyUserOnTransactionFailed = async (event, attendee) => {
    let attendeeName = utils.toTitleCase((attendee.first_name && attendee.last_name) ? attendee.first_name + " " + attendee.last_name : attendee.username);
    if (attendee.email && attendee.email !== "") {
        let tmpl = fs.readFileSync("./lambda/emailtemplates/notify_attendee_on_payment_failed_when_purchase_ticket.html", "utf8");
        let mailSubject = emailAndPushNotiTitles.TRANSACTION_FAILED_WHILE_BUYING_TICKET;
        let event_name = event.event_name.replace(/\s+/g, "-").toLowerCase();
        let event_url = `${process.env.UI_BASE_URL}/eventdetails/${event_name}/${event.event_id}`;
        // let paymentMethodLink = `${process.env.UI_BASE_URL}/payout_methods`;
        let templateVars = Object.assign({
            base_url: process.env.UI_BASE_URL,
            eventUrl: event_url,
            eventName: event.event_name,
...
//notify user on transaction failed when you tried to buy tickets for an event by push notification
const notifyUserOnTransactionFailedByPushNotification = async (event, attendee) => {
    let PAYMENT_FAILED_FOR_EVENT = emailAndPushNotiTitles.PAYMENT_FAILED_FOR_EVENT;
    PAYMENT_FAILED_FOR_EVENT = PAYMENT_FAILED_FOR_EVENT.replace('<eventName>', event.event_name);
    var attendeeDeviceTokenData = await getUserDeviceToken(attendee.user_id);
    if (attendeeDeviceTokenData.length > 0) {
        var { token } = attendeeDeviceTokenData[0];
        var message = {
            title: emailAndPushNotiTitles.TRANSACTION_FAILED,
            body: PAYMENT_FAILED_FOR_EVENT,
            payload: { messageFrom: stringMessages.PROMO_TEAM }
        };
        await AWSManager.sendPushNotification(message, token);
    }
    ...
    let saveObj = {
        ...
        notify_type: stringMessages.DATA,
        payload_data: JSON.stringify({ messageFrom: stringMessages.PROMO_TEAM }),
        notify_user_id: attendee.user_id,
        notify_text: PAYMENT_FAILED_FOR_EVENT
    };
...
const getCustomerGetwayDetails = async function (user_id) {
    return await MDBObject.getData("user_payment_gateway", "gateway_id", { "_user_id": user_id }).then(data => {
        return data;
    });
}

const getStripeChargeByPaymentIntent = async (paymentIntentId) => {
    let listOfCharges = await stripe.charges.list({
        limit: 3,
        payment_intent: paymentIntentId
    });
    return listOfCharges.data[0];
}
...
module.exports.handler = async function (event, context, callback) {
    ...
    try {
        ...
        let intentId = requestData.intent_id;
        ...
        let ticketsNotify = [];
        let ticketsDetails = [];
        await MDBObject.getData("event_master", "*", {
            event_id: requestData.event_id
        }).then(eventData => {
            eventDetails = eventData[0];
            return eventDetails;
        })
        await MDBObject.getData("user_master", "*", {
            user_id: eventDetails._user_id
        }).then(result => {
            eventOrgDetails = result[0];
            return eventOrgDetails;
        })

        await MDBObject.getData("user_master", "*", { user_id: user.id }).then(result => {
        ...
        for (let index = 0; index < apiData.length; index++) {
            ...
            // await ticketsDetails.find(async function (item, i) {
        ...
        if (totalPrice > 0) {
            ...
            await getCustomerGetwayDetails(user.id).then(async (customerGetwayDetails) => {
                if (customerGetwayDetails && customerGetwayDetails.length > 0) {
                    customerID = customerGetwayDetails[0].gateway_id;
                    if (requestData.token != "") {
                        // await updateCustomer(requestData, customerID);
                    }
                } else {
            ...
            stripePaymentResponse = await getStripeChargeByPaymentIntent(intentId).then(async stripeResponse => {
                //When the organizer receives payment for an event 
                await notifyOrganiserOnPaymentReceives(eventDetails, eventOrgDetails, eventAttendeeDetails, totalPrice.toFixed(2));
                return stripeResponse;
            }).catch(async (error) => {
                //notify user on transaction failed when you tried to buy tickets for an event
                await notifyUserOnTransactionFailed(eventDetails, eventAttendeeDetails);
                await notifyUserOnTransactionFailedByPushNotification(eventDetails, eventAttendeeDetails);
        ...
        return Promise.all(
            await apiData.map(async (apiDataLoop, index) => {
                ...
                if (("layout_id" in requestData) && ("tier_id" in apiDataLoop) && ("seats" in apiDataLoop)) {
                    ...
                    let removeUserHoldSeatsQuery = `DELETE FROM seat_holding_master
                        WHERE _seat_id IN (${allSeats}) and _tier_id = ${apiDataLoop.tier_id} and _layout_id = ${requestData.layout_id}`;
                ...
                let ticketInfo = await MDBObject.getData("event_tickets", "price, ticket_type", { ticket_id: apiDataLoop._ticket_id }).then((tckRes) => {
                    return tckRes[0];
                });
                if (requestData.formDetails && requestData.formDetails.length > 1) {
                ...
                for (let index = 0; index < apiDataLoop.ticket_qty; index++) {
                    let details_json = {
                        ...
                        _detail_form_id : (detailFormId && detailFormId.insertId) ? detailFormId.insertId : ((apiDataLoop.detailFormId && apiDataLoop.detailFormId.insertId) ? apiDataLoop.detailFormId.insertId : '')
                    };
                    ...
                    if (("layout_id" in requestData) &&
                        ("tier_id" in apiDataLoop) &&
                        ("seats" in apiDataLoop)) {
                    ...
                    await MDBObject.dataInsert("user_ticket_details", details_json).then(async (data) => {
                        let qrtext = {
                            ...
                            attendeeName: utils.toTitleCase((eventAttendeeDetails.first_name && eventAttendeeDetails.last_name) ? eventAttendeeDetails.first_name + " " + eventAttendeeDetails.last_name : eventAttendeeDetails.username),
                            ticket: userTicketId,
            ...
            await notifyUserByPushNotification(eventAttendeeDetails.user_id, eventDetails.event_name, totalQty);

            let PURCHASED_TICKETS_TO_EVENT = emailAndPushNotiTitles.PURCHASED_TICKETS_TO_EVENT;
            PURCHASED_TICKETS_TO_EVENT = PURCHASED_TICKETS_TO_EVENT
                .replace('<totalQty>', totalQty)
                .replace('<eventName>', eventDetails.event_name);
            ...
            let userSaveObj = {
                ...
                notify_user_id: eventAttendeeDetails.user_id,
                notify_text: PURCHASED_TICKETS_TO_EVENT,
            };
            await savePushNotification(userSaveObj);
