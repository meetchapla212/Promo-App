...
module.exports.handler = async function (event, context, callback) {
    ...
    try {
        ...
        let existingRecordsQuery = `SELECT ${fieldsObj} FROM user_tickets
            ...
            WHERE user_id = ${user.id} AND user_tickets.is_deleted = 0 AND event_master.is_deleted = 0 AND user_tickets.status = 'active' AND ticket_details_status != 'cancel' AND ticket_details_status != 'refunded'`;
        ...
        if (existingRecords && existingRecords.length) {
            await Promise.all(
                existingRecords.map(async ticketsArray => {
                    let ticketDetails = `SELECT event_tickets.*
                        FROM user_ticket_details JOIN event_tickets
            ...
            response = { success: true, message: successMessages.GET_ALL_TICKETS, data: existingRecords };
        } else {
