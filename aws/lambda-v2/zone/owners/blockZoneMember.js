...
module.exports.handler = async function (event, context, callback) {
    ...
    try {
        ...
        let isUserIsOwnerOfCurrentZone = await DBManager.getData('admin_zoneowner_invitation_requests', 'count(*) as totalCount', {
            user_id: user.id, zone_id: apiData.zone_id, request_status: 1, is_block: 0
        });
        if (!isUserIsOwnerOfCurrentZone[0].totalCount) {
            response = { success: false, message: errorMessages.NOT_OWNER_OF_THIS_ZONE_SO_UNABLE_TO_DELETE_ZONE_ATTENDEE };
            return awsRequestHelper.respondWithJsonBody(200, response);
        }

        await DBManager.dataUpdate("member_zonemember_invitation_requests", {
            is_block: apiData.is_block }, { user_id: member_id, zone_id: apiData.zone_id
        });
        if (apiData.is_block === false) {
            response = { success: true, message: successMessages.ZONE_MEMBER_UNBLOCKED }
        } else {
            response = { success: true, message: successMessages.ZONE_MEMBER_BLOCKED }
        }
