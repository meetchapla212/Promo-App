...
module.exports.handler = async function (event, context, callback) {
    ...
    try {
        ...
        let queryZoneListingCount = `SELECT count(*) as totalCount from admin_zoneowner_invitation_requests as AZIR join user_master as UM on AZIR.user_id = UM.user_id join zone_master as ZM on ZM.zone_id = AZIR.zone_id where UM.is_deleted = 0 AND AZIR.is_deleted = 0 AND AZIR.is_block = 0 AND AZIR.zone_id = ${zone_id}`;
        ...
        if (searchText) {
            ...
            queryZoneListingCount += ` AND (${searchQry})`;
            queryZoneListing += ` AND (${searchQry})`;
        }

        return await DBManager.runQuery(queryZoneListingCount).then(async (data) => {
            if (data && data.length && data[0].totalCount) {
                ...
                return DBManager.runQuery(queryZoneListing).then(async (data) => {
                    ...
                    response = { success: true, message: successMessages.GET_ZONE_OWNER_LIST, data: dataResponse }
