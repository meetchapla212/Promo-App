...
module.exports.handler = async function (event, context, callback) {
    ...
    try {
        ...
        let currentPassword = md5(apiData["currentPassword"]);
        let newPassword = md5(apiData["newPassword"]);
        let whereQry = { user_id: user["id"] };
        let existingRecords = await DBManager.getData("user_master", "password", whereQry);
        if (existingRecords && existingRecords.length) {
            if (currentPassword === existingRecords[0].password) {
                ...
                response.message = errorMessages.INCORRECT_PASSWORD;
            }
        }
