...
module.exports.handler = async function (event, context, callback) {
    ...
    try {
        ...
        let existingRecordsQuery = `SELECT otp FROM user_master where mobile_number = '${apiData.mobile_number}' AND country_code = '${countryCode}'`;
        let existingRecords = await DBManager.runQuery(existingRecordsQuery);
        if (existingRecords && existingRecords.length && existingRecords[0].otp) {
            if (apiData.otp && apiData.otp == existingRecords[0].otp) {
                ...
                response.message = "Failure, User otp does not verified correctly, please try again later!"
                return awsRequestHelper.respondWithJsonBody(200, response);
            }
        } else {
