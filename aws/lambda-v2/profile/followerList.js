...
module.exports.handler = async function (event, context, callback) {
    ...
    try {
        ...
        let whereQry = { follow_user_id: userId };
        ...
        if (existingRecords && existingRecords.length) {
            ...
            if (existingRecords && existingRecords.length) {
                response = { success: true, message: successMessages.GET_FOLLOWERS_LIST, data: existingRecords };
            } else {
                response = { success: true, message: errorMessages.NO_FOLLOWERS_FOUND }
            }
        } else {
            response = { success: true, message: errorMessages.NO_FOLLOWERS_FOUND }
        }
