...
module.exports.handler = async function (event, context, callback) {
    ...
    try {
        ...
        let whereQuery = { is_deleted: 0, status: 'active', is_draft: 0 };
        let customWhere = '';
        let currentDate = moment().format('YYYY-MM-DD HH:mm:ss');
        if (status == 'upcoming') {
            customWhere = ` AND event_master.start_date_utc > '${currentDate}' `
        } else {
            whereQuery = { is_draft: 0 }
            customWhere = ` AND event_master.end_date_utc > '${currentDate}'`
        }
        ...
        let QueryEvent = `(SELECT * FROM event_master WHERE ${wheryQry} AND _user_id = ${userId}  ${customWhere} ) UNION ALL (SELECT event_master.* FROM event_administrator JOIN event_master ON event_administrator._event_id = event_master.event_id WHERE ${wheryQry} AND event_administrator._user_id = ${userId} AND event_master.is_deleted = 0 ${customWhere}) ORDER BY start_date_time_ms LIMIT ${pageoffset},${pageLimit}`;
