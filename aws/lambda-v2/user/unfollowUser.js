...
//When a user unfollows the organizer of an event
const notifyOrganiserOnUsersUnFollow = async (user, organiserId) => {
    ...
    if (userDeviceTokenData.length > 0) {
        ...
        var message = {
            title: emailAndPushNotiTitles.USER_STOPPED_FOLLOWING,
            body: stringMessages.User + ' ' + userName + emailAndPushNotiTitles.USER_HAS_STOPPED_FOLLOWING_YOU,
            payload: {
    ...
    let saveObj = {
        ...
        notify_text: stringMessages.User + userName + emailAndPushNotiTitles.USER_HAS_STOPPED_FOLLOWING_YOU,
    }
...
module.exports.handler = async function (event, context, callback) {
    ...
    try {
        ...
        if (existingRecords && existingRecords[0] && existingRecords[0].totalCount) { 
            await DBManager.dataDelete('follow_users', whereQry, "AND")

            //When a user unfollows the organizer of an event
            await notifyOrganiserOnUsersUnFollow(loggedUserDetails, apiData.follow_user_id);

            response = { success: true, message: successMessages.USER_UNFOLLOWED };
        } else {
            response = { success: false, message: errorMessages.DATA_NOT_FOUND }
        }
